...
function printHelp ()
{
    ...
    echo -e "${GREEN}-n ${RESET}defaults to ${GREEN}accountingportal-network${RESET}. use ${YELLOW}-n your-named-network ${RESET}if you are using a different network name"  | indent
...
{
    echo ""
    echo -e "${YELLOW}archive creation script for the Accounting Portal" | indent
...
NETWORK_NAME="accountingportal-network"
...
rm -rf ../archive_files/$NETWORK_NAME.bna
cd ../network

composer archive create --sourceType dir --sourceName . -a ../archive_files/$NETWORK_NAME.bna
#
